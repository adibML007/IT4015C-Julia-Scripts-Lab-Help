...
X = df[1:100, 1]  # Get the first column
# xt = collect(range(1, 100, 10))
indices = 0:100
xt = reduce(vcat, i for i in indices)
println(xt)
...
using Plots
# for i in 2:10
display(plot!(X, df[1:100, 7], seriestype = :line, title = "Real Earning vs. Time", xlabel = "Time", ylabel = "Real Earning", legend = false))
# end
readline()
